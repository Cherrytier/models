...
def crop_large_image(img_path, xml_path, output_path):
    image_cropper = ImageListCropper(img_path, xml_path, output_path)
    image_cropper.update()
    image_list, _ = create_file_list(output_path)
    return image_list
    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph, config=config) as sess:
            ...
            for idx, paths in enumerate(image_lists):
                print(
                    'predicting {} of {} images'.format(idx, len(image_lists)))
